#!/bin/bash
# ──────────────────────────────────────────────────────────────────
#  Eval-only pass for the SEMI-AR masked-diffusion run:
#    runs/clevr/masked_diff/ours_text_diffhead_clip_out16_tokdrop1.0_semiar
#
#  90-sample training eval set, NO visualization artifacts — only
#  eval_scores.json / eval_summary.json. Semi-AR activated via
#  run_config.json.
#
#  Usage:
#    bash script/eval_only_masked_diff_generation_semiar.sh
