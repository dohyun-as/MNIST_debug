...
# ──────────────────────────────────────────────────────────
#  3) Conditional AR Fine-tuning from step50000
#     Resume from step50000, train 10 more steps (50000→50010)
#     Eval every step, greedy sampling (temperature=0)
# ──────────────────────────────────────────────────────────

OUTPUT_DIR="./outputs/outputs_ar/ar_top1"
mkdir -p "${OUTPUT_DIR}"
...
COMMON_ARGS="--output_dir ${OUTPUT_DIR} \
  ...
  --log_with tensorboard \
  --grad_accum_steps 1 \
  --resume_dir "./outputs/outputs_ar/conditional/ckpt/step50000" \
"
