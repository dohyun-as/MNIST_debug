#!/bin/bash
# ──────────────────────────────────────────────────────────────────
#  CLEVR 256×256 — Single-Resolution (8×8 only) DiT + ViT encoder
#  (continuous features, no FSQ)
# ──────────────────────────────────────────────────────────────────
#
#  Multi-resolution 대신 finest 8×8 한 레벨만 사용하는 variant.
#  --level_sizes 8 로 encoder 가 8×8 한 레벨만 생성하고,
#  --no_level_drop 으로 level drop 도 비활성.
#
#  Usage:
#    bash script/train_clevr_dit_our_continuous_only8x8.sh
#    GPUS=0,1 bash script/train_clevr_dit_our_continuous_only8x8.sh
...
accelerate launch \
    ...
    --backbone dit \
    --output_dir runs/clevr/backbone/256_dit_vit_flow_cont_out16_only8x8_tokdrop1.0 \
    --train_dir "$CLEVR_DIR" \
    --val_dir "$CLEVR_VAL" \
