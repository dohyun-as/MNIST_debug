...
# ──────────────────────────────────────────────────────────────────
#
#  기존 random_drop 스크립트와 동일하되,
#  --cond_token_drop_linear 플래그를 켜서
#  drop ratio를 Uniform(0,1) 대신 linear(f(p)=2p) 분포에서 샘플링.
#  → 높은 masking ratio가 더 자주 뽑힘.
#
#  Usage:
#    bash script/train_clevr_dit_our_continuous_random_drop_linear.sh
#    GPUS=0,1 bash script/train_clevr_dit_our_continuous_random_drop_linear.sh
...
accelerate launch \
    ...
    --backbone dit \
    --output_dir runs/clevr/backbone/out16_randomdrop_alllvl_linear_multi_res \
    --train_dir "$CLEVR_DIR" \
    --val_dir "$CLEVR_VAL" \
