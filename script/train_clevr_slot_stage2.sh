#!/bin/bash
# ──────────────────────────────────────────────────────────────────
#  CLEVR — STAGE 2: Masked Discrete Diffusion on SLOT tokens
#                  (continuous, MAR-style diffusion head, CLIP text)
#
#  Reference (existing): script/train_discrete_diffusion_clevr_ours_text_diffhead_clip.sh
#  Differences from reference:
#    1) entrypoint = src/train_discrete_diffusion_slot.py
#         (thin monkey-patch wrapper around train_discrete_diffusion_v2)
#    2) --pretrained_output_dir = stage-1 SLOT run (16 slots, K=16)
#    3) --pos_emb_type 1d
#         slots aren't on a spatial grid; multires/2d would be wrong.
#         Each slot index gets a learnable 1-D positional embedding.
#  Everything else (hyperparameters, eval, sampling, CLEVR cond eval,
#  detector/classifier-based alignment scoring) matches the reference.
#
#  Usage:
#    bash script/train_clevr_slot_stage2.sh
#    GPUS=0,1 PRETRAINED_DIR=runs/clevr/slot_stage1/256_slot16_d192 \
#      bash script/train_clevr_slot_stage2.sh
# ──────────────────────────────────────────────────────────────────
...
if [ ${NUM_GPUS} -gt 1 ]; then
  CUDA_VISIBLE_DEVICES=${GPUS} accelerate launch \
    ...
    src/train_discrete_diffusion_slot.py $COMMON_ARGS
else
  CUDA_VISIBLE_DEVICES=${GPUS} accelerate launch \
