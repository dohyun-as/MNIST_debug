#!/bin/bash
# ──────────────────────────────────────────────────────────────────
#  Visualize the CLEVR masked-diffusion run trained with
#  --semi_autoregressive (coarse-to-fine semi-AR):
#    runs/clevr/masked_diff/ours_text_diffhead_clip_out16_tokdrop1.0_semiar
#
#  Mirrors visualize_masked_diff_generation.sh — same CLI, outputs,
#  and 3-sampler sweep (ddpm_cache, confidence_top1, confidence_cosine).
#  The only difference: RUN_DIR defaults to the _semiar checkpoint.
#
#  How semi-AR is activated for sampling:
#    visualize_masked_diff_generation.py reads
#    `semi_autoregressive` from run_config.json and calls
#    `diffusion.set_semi_ar(level_idx, n_levels)`. That triggers
#    sample_continuous → _sample_continuous_semi_ar (coarse→fine
#    outer loop, MDLM t-sweep inner loop). mask_history / return_history
#    are produced identically, so the generation-order heatmaps,
#    level-ablation, and grid-overlay artifacts all work unchanged.
#
#  Usage:
#    bash script/visualize_masked_diff_generation_semiar.sh
#    GPUS=0,1 bash script/visualize_masked_diff_generation_semiar.sh
#    STEP=100000 VARIANT=base bash script/visualize_masked_diff_generation_semiar.sh
# ──────────────────────────────────────────────────────────────────
...
RUN_DIR=${RUN_DIR:-"./runs/clevr/masked_diff/ours_text_diffhead_clip_out16_tokdrop1.0_semiar"}
STEP=${STEP:-50000}
VARIANT=${VARIANT:-"ema"}
